...
#                                                              #
#  Written by Matthew Gilliard, Jan 2011, and distributed      #
...
function do_doomsday(){
  ...
  if $CORRECT; then
    echo -ne "\033[1;32m:)\033[0m ($ATTEMPTS attempts)";
    EXIT_CODE=0;
  else
    if [ $ATTEMPTS -eq 7 ]; then
      echo -ne "\033[1;31m>:O\033[0m 7 wrong guesses!!?! ";
      echo -n  "Your an idiot"; #"ironic" misspelling
    else
